...
export class ObjectStorageService {
  ...
  async getObjectEntityUploadURL(bookingId?: string): Promise<string> {
    ...
    // Create organized folder structure: uploads/YYYY-MM-DD/bookingId_timestamp
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const timestamp = Date.now();
    const fileId = bookingId ? `${bookingId}_${timestamp}` : `temp_${randomUUID()}`;
  ...
  async getPublicImageUploadURL(): Promise<string> {
